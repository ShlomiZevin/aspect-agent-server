...
- A TREND OR TIMELINE MUST BE KEYED ON A REAL DATE. Reject any "week N" / "period N" / "month N" framing built from a per-entity ordinal — a ROW_NUMBER partitioned by item, store or customer. Each entity's "week 2" is then a DIFFERENT calendar week, so a total across them describes a period that never existed, and the trend is an artefact of the numbering rather than of the business. The aggregates will agree with the rows here, which is exactly why this needs saying: the arithmetic is right and the axis is meaningless. Measured case (2026-09-02): a report claimed units "peaked at 181,286 in week 2 and declined to 55,768 by week 6 — a 69% slide", every figure reconciling to the query, while the real calendar weeks ran 152,458 to 77,241.
