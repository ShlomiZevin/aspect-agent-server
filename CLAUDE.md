...
- `capability-gate.service` — deterministic regex pre-flight in `data-query.service.queryByQuestion`: unambiguous absent-dimension questions refuse in ~1ms BEFORE SQL generation; unresolved vocabulary is surfaced instead of hunted. Precision beats recall — ambiguity falls through to prompts.
...
- Crew "data discipline" — `CrewMember` takes optional `datasetSchema`; `buildContext()` injects `renderForCrew(manifest)` (answer-first sentence, never arithmetically combine user-quoted numbers with DB numbers, VAT-basis matching, clarify-only-exact-ambiguous-phrases), rendered by the dispatcher like persona. Opt-in is one line in the crew file.
...
**Replay real customer sessions as the regression bar.** `scripts/build-customer-corpus.js` freezes every real (anon_%) customer question into `verification/representative-dataset/customer-corpus.json` (purity hard-asserted); `run-customer-replay.js` replays them through the REAL chat path (`runChatTurn`) with per-turn resume and dataState snapshots; `compare-replays.js` diffs two runs and refuses when data moved (frozen-data rule). Raw run JSONs are gitignored — the stage task files in `tasks/done/` are the durable record. Stage-2/3 bar: 74/74 answered, 0 regressions, 100% basis + coverage disclosure on money answers.
