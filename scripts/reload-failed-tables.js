/**
 * Reload Failed Tables Script
 *
 * Reloads only the tables that failed during initial load
...
async function reloadFailedTables() {
  console.log('🔧 Reloading failed tables with corrected types...\n');
  console.log('Tables to reload:');
  FAILED_TABLES.forEach(t => console.log(`  - ${t}`));
  console.log();
  ...
  try {
    ...
    // Filter only failed tables
    const schemas = allSchemas.filter(s => FAILED_TABLES.includes(s.tableName));

    console.log(`📋 Found ${schemas.length} tables to reload\n`);
    ...
    for (let i = 0; i < schemas.length; i++) {
      ...
      try {
        ...
        // Step 2: Recreate table with NUMERIC for all numeric columns
        console.log(`  🏗️  Creating table with corrected types...`);
    ...
    console.log('✅ Failed tables reload complete!\n');
...
/**
 * Generate CREATE TABLE SQL with TEXT for all columns
 * SAFEST approach - handles decimals, percentages, and any other edge cases
 */
function generateCreateTableSQL(schema) {
  const tableName = schema.tableName;
  const columns = schema.columns;

  // Use TEXT for ALL columns to avoid any type mismatch issues
  // PostgreSQL can handle numeric operations on TEXT columns if needed
...
module.exports = { reloadFailedTables };
