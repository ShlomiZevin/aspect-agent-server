/**
 * The Stock Reload — two-phase zero-downtime reload.
 * Architecture mirrors reload-newdeli.js exactly.
 *
 * Phase 1 — loadTheStock(targetSchema, emitLog):
 *   Scan GCS → read CSV headers → create UNLOGGED tables → COPY data
 *
 * Phase 2 — indexTheStock(targetSchema, emitLog):
...
const { buildColumnLookup } = require('./column-aliases-thestock');
...
const FILE_TO_TABLE = {
  ...
  'C100_CSV.csv':              'inventory_c100',
  'Taarichim_CSV.csv':         'calendar',
  'Taarichim_Compare_CSV.csv': 'calendar_compare',
};
...
async function loadTheStock(targetSchema, emitLog, options = {}) {
  let filesLoaded = 0;
  let totalRows = 0;
  const fileResults = [];
  ...
  const importMonths = options.importMonths != null
    ...
    : (parseInt(process.env.THESTOCK_IMPORT_MONTHS || '0', 10) || 0);
  if (importMonths > 0) {
...
async function indexTheStock(targetSchema, emitLog) {
...
async function getTheStockDataInfo() {
  const pool = getPool();
  try {
