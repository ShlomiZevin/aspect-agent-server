...
const SYSTEM_PROMPT = [
  ...
  'When the change asks for a field with fixed allowed values ("choice',
  'field", "options", "enum field", "one of X/Y/Z"), emit BOTH pieces:',
  ...
  '  value: "..." }, ...] }`. If `<fieldname>_choices` is already taken by',
  '  ANOTHER enum, suffix `2`, `3`, … NEVER encode the allowed values in',
  ...
  '  two-target apply: the crew target returns `fields` (the FieldDef',
  '  referencing the enum id), the agent target returns `enums` (the',
  '  owned EnumTypeDef). Never invent a `crew.enums` section.',
