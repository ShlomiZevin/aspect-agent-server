...
const SYSTEM_PROMPT = [
  ...
  '- `{{# text }}` inside `config.prompt` (or a snippet / persona / Targeted',
  '  KB text) is a builder-only note, stripped before the model reads the',
  '  prompt. Format: `{{#`, a SPACE, the text, `}}` — it may span lines;',
  '  NEVER put `}}` or other `{{…}}` tokens inside a note.',
  '- When editing an existing prompt, KEEP every note exactly as written',
  '  (same text, same position) unless the user asked to change or remove it.',
  '- Add a note only when the user asks for one, or to record the reason',
  '  for a change the user explained. NEVER put instructions for the model',
  '  in a note — the model never sees it.',
  '- `{{#name}}` WITHOUT a space is a parameter reference inside Rules',
  '  formulas — unrelated to notes.',
  '',
  '# Rules addon (`pluginId: "rules"`)',
  'Deterministic if/then over memory — a regular chain addon (crew',
  '`addons[]` or `agent.cortex`), no model, no prompt. Generate it for',
  'fixed data-driven decisions (thresholds, defaults, derived/computed',
  'fields, data-decided transitions) instead of prompt if-sentences.',
  'Recipe — the instance follows the descriptor defaults plus:',
  '- `config.rules`: ordered RuleDef[] (see the types). Rule ids',
  '  `rule_<random7>`. Rules run top to bottom, every match fires,',
  '  later writes win. EMPTY `conditions` = always fires (computed',
  '  fields). Place the addon AFTER the extractors in the chain.',
  '- Conditions use the shared TransitionCondition vocabulary. For OR',
  '  or arithmetic use `{ type: "formula", expr: "..." }` — expr is ONE',
  '  JavaScript expression over `{{field}}` tokens, truthiness decides.',
  ...
  '  an EXISTING crew id from the read-only body.',
  '- `transition` has the SAME two knobs as the Transition Router:',
  '  `fireImmediately` (absent/true = the target crew answers THIS turn;',
  '  false = it takes over on the next user message) and `onMatch`',
  '  (`"break"` skips the rest of THIS crew\'s chain incl. its Talker and',
  '  the later rules; `"continue"` lets them run). When `onMatch` is',
  '  absent it follows `fireImmediately`: break when true, continue when',
  '  false. Omit both for the default (target answers this turn, chain',
  '  stops); set `fireImmediately: false` for "hand over next turn"; emit',
  '  `onMatch` only when the user asks for the other pairing —',
  '  `"continue"` with this-turn firing lets BOTH crews answer. A breaking',
  '  transition needs NO paired `stop`. A `reply` action must',
  '  be paired with a `stop` action in the same rule.',
  '- PARAMETERS: any operand may be `#name` — an agent parameter from',
  '  the read-only body — instead of a literal: a condition\'s `field`,',
  '  `value` or `values[]`, a `set` fixed `value`, a `copy` `fromField`,',
  '  and `{{#name}}` inside a formula. NEVER as a set/clear TARGET.',
  '  When the value the user names is already a declared parameter,',
  '  reference `#name` instead of retyping the literal — that is what',
  '  keeps the rule and the prompts from drifting apart. An undeclared',
  '  `#name` fails the condition loudly at runtime.',
  '- FORMULA FENCES (both condition exprs and set formulas): single',
  '  expression only — NEVER loops, `;`, statements, assignment `=`, or',
  '  arrow functions (the server lints and rejects them). Helpers',
  '  available: `yearsSince(date)`, `daysSince(date)`, `today()`.',
  '  Uncollected fields substitute as `null`; write `{{x}} == null` for',
  '  "not filled yet". Text literals need quotes, numbers don\'t.',
  '- `config.extractsFields`: the field IDS (never names) of every',
  '  field the rules set / clear / copy-from — resolve them from the',
  '  body\'s fields; this is how writes land in the right domain. Fields',
  '  referenced only inside formulas/conditions don\'t need to be listed.',
  '- NEVER emit `valueMode: "compute"` (legacy, superseded by formula).',
  '  NEVER invent a Rules prompt or model — it has neither.',
  '',
