...
const SYSTEM_PROMPT = [
  ...
  '  field", "options", "one of X/Y/Z"): the value list is a real enum',
  '  that lives ONLY on the agent body (`agent.enums`), while the FieldDef',
  '  lives wherever the field is scoped. Decompose accordingly:',
  '  - Agent-scoped field → ONE agent target. Its what_to_do must say',
  '    BOTH things explicitly: create the enum (named <fieldname>_choices,',
  '    owned by the new field, with the exact values) AND create the field',
  '    (type enum, bound to that enum).',
  '  - Crew-scoped field → TWO targets, agent first. Agent target:',
  '    "create enum <fieldname>_choices on agent.enums with values ...".',
  '    Crew target: "add field <name> (type enum) bound to the enum NAMED',
  '    <fieldname>_choices from the agent" — reference the enum BY NAME,',
  '    its id is minted during generation.',
