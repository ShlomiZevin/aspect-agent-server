...
const SYSTEM_PROMPT = [
  ...
  '  about it" → exclude it.',
  '- CHOICE FIELDS (a field with a fixed list of allowed values — "choice',
  '  field", "options", "one of X/Y/Z", "simple list"): this is a CHOICE —',
  '  a minimal field-owned value list — NOT a Targeted KB. Say the word',
  '  "Choice" explicitly in the what_to_do and add: "minimal owned value',
  '  list — no sections, no umbrella texts". A full Targeted KB (per-value',
  '  guidance / dynamic context) is only for when the user explicitly',
  '  asked for prompt content that changes by value.',
  '  Scope + decomposition:',
  '  - DEFAULT: agent-scoped → ONE agent target. Its what_to_do must say',
  '    BOTH things explicitly: create the Choice enum (named',
  '    <fieldname>_choices, owned by the new field, with the exact values,',
  '    minimal) AND create the field (type enum, bound to that enum).',
  '    Prefer this unless the user explicitly wants the field crew-only.',
  '  - Crew-scoped (only when explicitly requested) → TWO targets, agent',
  '    first. Agent target: "create Choice enum <fieldname>_choices on',
  '    agent.enums with values ... (minimal)". Crew target: "add field',
  '    <name> (type enum) bound to the enum NAMED <fieldname>_choices',
  '    from the agent" — reference the enum BY NAME, its id is minted',
  '    during generation.',
