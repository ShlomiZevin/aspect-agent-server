...
/**
 * GET /api/dynamic-kb/:agentName/files
 * List all dynamic files for an agent
 */
app.get('/api/dynamic-kb/:agentName/files', async (req, res) => {
  try {
    const agentId = await crewMembersService.getAgentId(req.params.agentName);
    if (!agentId) return res.status(404).json({ error: 'Agent not found' });
    const files = await dynamicKBService.getFilesByAgent(agentId);
    res.json(files);
  } catch (err) {
    console.error('[DynamicKB] List files error:', err.message);
    res.status(500).json({ error: err.message });
  }
});

/**
 * POST /api/dynamic-kb/:agentName/files
 * Create a new dynamic file record
 * Body: { name, fileType }
 */
app.post('/api/dynamic-kb/:agentName/files', async (req, res) => {
  try {
    ...
    if (!['text', 'table'].includes(fileType)) {
      return res.status(400).json({ error: 'fileType must be text or table' });
    }
    const agentId = await crewMembersService.getAgentId(req.params.agentName);
    if (!agentId) return res.status(404).json({ error: 'Agent not found' });
    const file = await dynamicKBService.createFile(agentId, name, fileType);
    res.status(201).json(file);
  } catch (err) {
    console.error('[DynamicKB] Create file error:', err.message);
    res.status(500).json({ error: err.message });
  }
});

/**
 * GET /api/dynamic-kb/files/:fileId
 * Get a single dynamic file record
 */
app.get('/api/dynamic-kb/files/:fileId', async (req, res) => {
  try {
    ...
    console.error('[DynamicKB] Get file error:', err.message);
    res.status(500).json({ error: err.message });
  }
});

/**
 * PUT /api/dynamic-kb/files/:fileId
...
app.put('/api/dynamic-kb/files/:fileId', async (req, res) => {
  try {
    ...
    const file = await dynamicKBService.updateFile(parseInt(req.params.fileId), { name });
    res.json(file);
  } catch (err) {
    console.error('[DynamicKB] Update file error:', err.message);
    res.status(500).json({ error: err.message });
  }
});

/**
 * DELETE /api/dynamic-kb/files/:fileId
 * Delete a dynamic file (detaches from all KBs, removes from GCS)
 */
app.delete('/api/dynamic-kb/files/:fileId', async (req, res) => {
  try {
    ...
    console.error('[DynamicKB] Delete file error:', err.message);
    res.status(500).json({ error: err.message });
  }
});

/**
 * GET /api/dynamic-kb/files/:fileId/content
 * Load file content from GCS
 */
 ...
 * Save file content and auto-sync to attached KBs
...
app.put('/api/dynamic-kb/files/:fileId/content', async (req, res) => {
  try {
    ...
    const result = await dynamicKBService.saveContent(parseInt(req.params.fileId), content, fileType);
    res.json(result);
  } catch (err) {
...
/**
 * GET /api/dynamic-kb/files/:fileId/attachments
...
app.get('/api/dynamic-kb/files/:fileId/attachments', async (req, res) => {
  try {
    ...
    console.error('[DynamicKB] Get attachments error:', err.message);
    res.status(500).json({ error: err.message });
  }
});

/**
 * POST /api/dynamic-kb/files/:fileId/attach/:kbId
 * Attach a dynamic file to a KB
 */
app.post('/api/dynamic-kb/files/:fileId/attach/:kbId', async (req, res) => {
  try {
    const result = await dynamicKBService.attachToKB(
...
/**
 * DELETE /api/dynamic-kb/files/:fileId/attach/:kbId
 * Detach a dynamic file from a KB
 */
app.delete('/api/dynamic-kb/files/:fileId/attach/:kbId', async (req, res) => {
  try {
    await dynamicKBService.detachFromKB(parseInt(req.params.fileId), parseInt(req.params.kbId));
    res.json({ success: true });
  } catch (err) {
...
/**
 * POST /api/dynamic-kb/import/doc
 * Upload a .doc/.docx file and return extracted plain text
 */
app.post('/api/dynamic-kb/import/doc', upload.single('file'), async (req, res) => {
  try {
    if (!req.file) return res.status(400).json({ error: 'No file uploaded' });
    const text = await dynamicKBService.parseDocx(req.file.buffer);
    res.json({ text });
  } catch (err) {
    console.error('[DynamicKB] Import doc error:', err.message);
    res.status(500).json({ error: err.message });
  }
});

/**
 * POST /api/dynamic-kb/import/spreadsheet
 * Upload a .csv/.xls/.xlsx file and return parsed table JSON
 */
app.post('/api/dynamic-kb/import/spreadsheet', upload.single('file'), async (req, res) => {
  try {
    if (!req.file) return res.status(400).json({ error: 'No file uploaded' });
    const result = dynamicKBService.parseSpreadsheet(req.file.buffer, req.file.mimetype);
    res.json(result);
  } catch (err) {
    console.error('[DynamicKB] Import spreadsheet error:', err.message);
    res.status(500).json({ error: err.message });
  }
});
