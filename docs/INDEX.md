...
- **BUILDER_V2_TRIGGERS.md** — Planning doc for Triggers (proactive): agents that act with no user message. Agent-level `agent.triggers[]` — a third surface beside Live Brain / Profiler, **not** an addon. One system clock, per-type clause declarations shared by the tick and the explainer, `trigger_events` with five outcomes, `LISTEN/NOTIFY` live push. First type: Silence.
